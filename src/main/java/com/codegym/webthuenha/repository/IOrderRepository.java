...
public interface IOrderRepository extends JpaRepository<Order, Long> {
    @Query(nativeQuery = true, value = "select * from orders where house_id = :id and (CURDATE() <= :startTime or CURDATE() <= :endTime) and ((orders.order_status_id = 2 or orders.order_status_id =3)  and ((orders.start_time >= :startTime and orders.start_time >= :endTime) or (orders.end_time <= :startTime and orders.end_time <= :endTime)))")
